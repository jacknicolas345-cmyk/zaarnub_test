    ...
    <div className="container mx-auto px-4 py-16 max-w-5xl">
      ...
      <form onSubmit={submit} className="bg-card rounded-2xl p-6 md:p-8 shadow-luxe space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div><Label>ایمیل *</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required /></div>
          <div><Label>موبایل</Label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} maxLength={11} /></div>
          <div><Label>موضوع</Label><Input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} /></div>
        </div>
        <div><Label>پیام *</Label><Textarea rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required /></div>
        <Button type="submit" disabled={loading} className="bg-gradient-gold text-primary-foreground hover:opacity-90">
          <Send className="ml-2 h-4 w-4" /> {loading ? "در حال ارسال..." : "ارسال پیام"}
        </Button>
      </form>
    </div>
